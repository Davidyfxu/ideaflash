  ...
  updateNote: async (id, updates) => {
    ...
    try {
      ...
      const updatedNotes = currentNotes.map((note) =>
        note.idea_flash_id === id ? updatedNote : note,
      );
      ...
      if (isApiEnabled) {
        try {
          ...
          const ideaFlashId = updatedNote.idea_flash_id || id;
    ...
    try {
      ...
      if (isApiEnabled) {
        try {
          ...
          const ideaFlashId = noteToDelete?.idea_flash_id || id;
    ...
    try {
      ...
      for (const apiNote of apiNotes) {
        ...
        const existingNote = localNotes.find(
          (note) =>
            note.idea_flash_id === apiNote.idea_flash_id ||
            note.id === apiNote.id,
        );
        ...
        if (!existingNote) {
          ...
          await database.updateNote(existingNote.id, {
            idea_flash_id: apiNote.idea_flash_id,
          });
        }
      }
